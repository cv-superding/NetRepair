...
- **14 项修复能力**，覆盖 95% 的「能连 Wi‑Fi 但上不了网」场景：
  - 默认勾选（8 项）：刷新 DNS 缓存、清理 ARP/NetBIOS 缓存、修复网络系统服务、重置 Winsock、重置 TCP/IP 协议栈、修复默认网关、恢复 DNS 自动获取、重置浏览器代理
  - 默认不勾选（6 项）：修复 Internet 选项（安全）、优化 TCP 全局参数（常规）、重置 Hosts 文件、重启网络适配器、重置 Windows 防火墙、清理路由表残留（后四项为谨慎级，需手动确认）
...
1. **一键修复（首页）**：列出 14 项修复能力，每项标注风险等级（安全 / 常规 / 谨慎）。可点击「智能勾选」（先跑诊断、自动勾选异常项），或对单项点击「单独修复」。
2. **网络诊断**：运行 10 项双路探测，逐项显示 ✅/❌ 状态与病因结论；异常项可直接一键跳转修复页并自动勾选。
3. **运行日志**：实时查看每条命令的执行结果（按级别着色），支持复制全部、清空显示与打开日志目录。
4. **备份还原**：查看与管理系统生成的配置备份（Hosts、代理设置、防火墙策略导出等）；Hosts 与代理备份支持一键还原，防火墙策略导出（.wfw）需手动导入。
5. **关于**：版本信息、作者信息（邮箱一键复制/发送）、外观设置（明暗主题 + 主题色，选择自动记忆）、数据目录自定义、开源许可与使用须知。

> 💡 **提示**：标记为「谨慎」的修复项会清除自定义配置或短暂中断网络（Hosts、防火墙策略、路由表、网卡重启等），其中 Hosts 与防火墙策略执行前会自动备份，确认无误再点。
...
│   ├── theme.py            # 旧版自制界面的莫奈配色方案（当前 ui.py 基于 QFluentWidgets，未引用，保留备用）
│   ├── anim.py             # 旧版自制界面的悬停动效组件（当前未引用，保留备用）
...
可在「运行日志」页与「备份还原」页一键打开所在文件夹；在「关于 → 数据目录」中可自定义日志与备份的存放位置（立即生效，已有文件不自动迁移）。
...
**Q：为什么需要管理员权限？**
A：修复网络配置（重置 Winsock/TCP 协议栈、修改 Hosts、防火墙策略等）本质是修改系统设置，必须由管理员执行；未提权时程序仍可启动，但需提权的修复项会执行失败，界面会给出提示。
